...
const Dashboard = (props) => {
  ...
  async function createMentoring(data_to_meet = "2020/11/28T19:30:28") {
    ...
    const response = api
      .get("insert_data_meeting/?meeting_hour=" + meeting_hour)
      .then(function () {
    ...
    if (user_kind == "Student") {
      return (
        <div className="option-dashboard-pane">
          <button onClick={() => push("/teamcreationpage")}>
            {idTeam == 0 ? "Criação de time" : "Alteração"}
          </button>
          <button onClick={() => push("/mentoringselecionpage")}>
            Seleção de mentorias
          </button>
          {idTeam != 0 ? (
            <button onClick={() => insertNewMemberForTeam(email)}>
              Inserir membro
            </button>
          ) : null}
          {idTeam != 0 ? (
          ...
          {idTeam != 0 ? (
            <button onClick={() => sendprojectlink(project_link)}>
              Enviar projeto
            </button>
          ) : null}
          {idTeam != 0 ? (
            <input onChange={(e) => set_project_link(e.target.value)} />
          ) : null}
        </div>
      );
      ...
      return (
        <div className="option-dashboard-pane">
          <textarea
            value={data_info}
            onChange={(e) => set_data_info(e.target.value)}
          />
          <button onClick={() => createMentoring(data_info)}>
            Criar mentoria
          </button>
        </div>
      );
              ...
              return (
                <li key={dataset.id}>
                  <div>
                    ...
                    />
                    <button onClick={(e) => sendNote(note, dataset.id)}>
                      Enviar nota
                    </button>
                  </div>
                </li>
      ...
      return (
        <ul>
          {property.map((e) => (
            <li key={e.id}>
              <p>
                Nome do mentor : <strong>{e.mentor_name}</strong>
              </p>
              <p>
                data da mentoria : <strong>{e.data_meeting}</strong>
              </p>
            </li>
          ))}
        </ul>
  ...
  function Template() {
    if (user_kind == "Student") {
      return (
        <div className="outlet-dashboard-template">
          <div className="template-dashboard">
            <ul>
              <li key={1}>
                {idTeam == 0
                  ? "Sem equipe definida"
                  : "O número da sua equipe é "}{" "}
                <strong>{idTeam}</strong>
              </li>
              <li key={2}>
                Desafio: <strong>{challenge_name}</strong>
              </li>
            </ul>
          </div>
          <div className="template-dashboard">
