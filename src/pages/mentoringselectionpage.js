...
const Mentoringselecionpage = (props) => {
    ...
    async function selectMentoring(chl){
        ...
        const uri = "select_mentor/?mentor_id=" +
        queries.mentor_id + "&mentor_name=" + queries.mentor_name + "&data_meeting=" + queries.data_meeting;
    ...
    function handleChallenge (chl) {
        return (
            <div className = "challengeBox">
                <h2>ID do mentor : {chl.mentor_id}</h2>
                <h2>Nome do mentor : {chl.mentor_name}</h2>
                <h2>Horário da mentoria : {chl.data_meeting}</h2>
                <button onClick = {() => selectMentoring(chl)}>Selecionar mentoria</button>
            </div>
        )
    }
    ...
    function filterMentors(dataaxios){
        ...
        const response = data_filtered.map(chl => <li key = {chl.id}> {handleChallenge(chl)} </li>);
